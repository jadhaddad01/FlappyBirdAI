...
class Button:
    def __init__(self, x, y, w=0, h=0, param_options={}):
        ...
        for key, value in param_options.items():
            if key not in options:
                raise KeyError(
                    key + " is not an option, is it spelt correctly")
    ...
    def _Generate_images(self):
        ...
        if self.image == None:
            ...
            self.image.blit(curve_square(
                self.w, self.h, self.curve, self.background), (0, 0))
            ...
            if not self.outline is None:
                pygame.draw.rect(self.hover_image, (0, 0, 0, 255),
        ...
        if self.enlarge:
            size = (int(self.w * self.enlarge_amount),
        ...
        if self.text != "":
            ...
            self.image.blit(txt, ((self.w - txt.get_width()) //
                                  2, (self.h - txt.get_height())//2))
            if self.enlarge:
                txt = self.enlarge_font.render(
                    self.text, True, self.text_colour)
            self.hover_image.blit(txt, ((self.hover_image.get_width(
            ) - txt.get_width())//2, (self.hover_image.get_height() - txt.get_height())//2))
        ...
        if self.hover_image.get_width() != self.w or self.hover_image.get_height() != self.h:
            self.enlarge = True
            self.dx, self.dy = self.hover_image.get_width(
            ) - self.w, self.hover_image.get_height() - self.h
    ...
    def _draw(self):
        ...
        if self.hover:
            if self.enlarge:
                self.surface.blit(self.hover_image,
    ...
    def key_down(self, e):
        ...
        if e.unicode == "":
            ...
            if len(self.text[self.current_line]) == 0:
                ...
                self.text[self.current_line] = self.text[self.current_line][:self.current_col -
                                                                            1] + self.text[self.current_line][self.current_col+1:]
                self.current_col -= 1
            ...
            self.text[self.current_line] = self.text[self.current_line][:self.current_col] + \
                e.unicode + self.text[self.current_line][self.current_col:]
            self.current_col += 1
            #wrapper
            ...
            self.current_line += 1 if self.current_line < len(
                self.text)-1 else 0
            self.current_col = min(self.current_col, len(
            ...
            self.current_col += 1 if len(
                self.text[self.current_line]) > self.current_col else 0
    ...
    def wrapper(self, change_cur=False):
        for cur_line, line in enumerate(self.text):
            for i in range(len(''.join(line))):
                ...
                if length > self.w:
                    indexs = [i for i, e in enumerate(
                        self.text[cur_line][:i]) if e == " "]
                    if cur_line < self.lines - 1:
                        ...
                        if cur_line < len(self.text):
                            ...
                            self.text[cur_line + 1] = self.text[cur_line][indexs[-1] +
                                                                          1:] + self.text[cur_line]
    ...
    def _draw(self):
        ...
        for line, text in enumerate(self.text):
            if len(text) != 0:
                ...
                self.surface.blit(
                    obj, (self.x + self.margin, self.y + (self.h*line)))
        #draw cursor
        if self.selected:
            if self.cursor:
                total = 0
            ...
            if mx > self.x and mx < self.x + self.w and my > self.y and my < self.y + self.h:
                if click:
                    if not self.prev_clicked:
                        ...
                        clicked_on = True
                        print(
                            "now selected" if self.selected else "not selected", self.text)
            if not clicked_on:
                if click:
                    if not self.prev_clicked:
                        self.selected = False
        ...
        if isinstance(lines, tuple):
            if lines[0] < 0 or lines[0] > self.lines or lines[1] < 0 or lines[1] > self.lines or lines[0] > lines[1]:
                raise IndexError("line index is out of range: " +
                                 str(lines) + " (0, " + str(str(self.lines)))
            string = []
...
class CheckBox:
    ...
    def _draw(self):
        #if outline
        if self.outline:
            pygame.draw.rect(self.surface, self.out_col,
    ...
    def update(self):
        ...
        if self.clicked_on:
            if self.direction == 'horizontal':
                ...
                self.slider_rect.x = self.x + \
                    self.val * (self.w - self.slider_w)
            else:
                self.val = (mouse_pos[1] - self.y) / self.h
                self.val = max(min(self.val, 1), 0)
                self.val = self._get_val(self.val)
                self.slider_rect.y = self.y + \
                    self.val * (self.h - self.slider_h)
        if not click:
            self.clicked_on = False
